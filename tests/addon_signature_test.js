...
var test_addon_install = async function(install, should_pass, expected_result, debug_messages) {
    try {
...
var run_test = async function(args, response_cb) {
    try {
        ...
        let unsigned_install = await AddonManager.getInstallForURL(unsigned_addon_url);

        let unsigned_addon_pass = await test_addon_install(unsigned_install, false, AddonManager.ERROR_SIGNEDSTATE_REQUIRED, debug_messages);
        if (!unsigned_addon_pass) {
            debug_messages.push("unsigned addon test failed");
        }
        let signed_addon_pass = await test_addon_install(signed_install, true, AddonManager.SIGNEDSTATE_SIGNED, debug_messages);
        ...
        return response_cb(test_passes, {origin: "run_test", debug_messages: debug_messages});
    } catch (e) {
